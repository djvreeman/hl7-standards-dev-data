...
def get_jira_workflow_info():
    ...
    # Done-terminal: terminal outcomes used for "Published share among Done-terminal" metrics
    # (same membership as done_states; explicit aliases for callers)
    done_terminal_status_ids = list(done_states)
    done_terminal_status_names = list(done_state_names)
    ...
    return {
        ...
        'done_terminal_status_ids': done_terminal_status_ids,
        'done_terminal_status_names': done_terminal_status_names,
...
def parse_history_created_timestamp(created_str):
    """Parse JIRA changelog `created` string to UTC pandas Timestamp, or None."""
    if not created_str:
        return None
    try:
        if '+' in created_str or str(created_str).endswith('Z'):
            return pd.Timestamp(created_str)
        return pd.Timestamp(created_str).tz_localize('UTC')
    except Exception:
        return None


def iter_status_transitions_chronological(history_data):
    """
    Return status transitions sorted by changelog time ascending.
    Each tuple is (created_ts, item_dict) where item has field status.
    """
    ...
    for history_entry in history_data:
        created_date = parse_history_created_timestamp(history_entry.get('created', ''))
        if created_date is None:
            continue
        for item in history_entry.get('items', []) or []:
            if item.get('field') == 'status':
                rows.append((created_date, item))
    rows.sort(key=lambda x: x[0])
    return rows


def is_transition_to_done_terminal(to_status_id, to_status_name, workflow_info):
    """True if transition target is Published, Duplicate, or Resolved - No Change."""
    canonical = None
    if to_status_id is not None and str(to_status_id).strip() != '':
        canonical = workflow_info['statuses'].get(str(to_status_id).strip())
    if canonical and canonical in workflow_info['done_terminal_status_names']:
        return True
    to_name = (to_status_name or '').strip()
    if not to_name:
        return False
    allowed = {n.lower() for n in workflow_info['done_terminal_status_names']}
    return to_name.lower() in allowed


def extract_first_done_terminal_transition_date(history_data, workflow_info):
    """
    First time (chronological) the issue enters a Done-terminal status:
    Published, Duplicate, or Resolved - No Change.
    """
    for created_date, item in iter_status_transitions_chronological(history_data):
        to_id = item.get('to')
        to_name = item.get('toString', '')
        if is_transition_to_done_terminal(to_id, to_name, workflow_info):
            return created_date
    return None
...
def process_data(df, analysis_periods, history_json_file=None):
    ...
    if history_col or history_data_from_json:
        ...
        df['First Done Terminal Date'] = df.apply(
            lambda row: extract_first_done_terminal_transition_date(
                get_history_for_row(row), workflow_info
            ),
            axis=1,
        )
        first_done_terminal_count = df['First Done Terminal Date'].notna().sum()
        print(f"  Extracted First Done Terminal Date for {first_done_terminal_count} issues")
        ...
        df['First Done Terminal Date'] = None
...
def get_performance_band_for_display(p80_value, display_decimals=0):
    """
    Return the performance band label using the same rounding semantics as the displayed P80.
    
    Why this exists:
    - The report typically *displays* P80 as an integer (rounded).
    - If the band is calculated using the unrounded P80 but displayed as rounded, boundary
      values (e.g., 60.4 shown as "60") can appear inconsistent (band might be Allegro
      while P80 appears to be in the Presto range).
    """
    if p80_value is None or (isinstance(p80_value, float) and pd.isna(p80_value)):
        return "N/A"
    if display_decimals == 0:
        return get_performance_band(float(round(p80_value)))
    return get_performance_band(float(round(p80_value, display_decimals)))
...
def _normalize_status_value_for_done_share(val, workflow_info):
    """Map Status cell to canonical workflow name or original string."""
    if pd.isna(val):
        return None
    s = str(val).strip()
    if s.lower() in ("nan", "none", ""):
        return None
    if s.isdigit():
        return workflow_info["statuses"].get(s, s)
    mapping = {
        "Resolved - change required": "Resolved - Change Required",
        "resolved - change required": "Resolved - Change Required",
        "Resolved - no change": "Resolved - No Change",
...
def _status_is_published(val, workflow_info):
    if pd.isna(val):
        return False
    s = str(val).strip()
    if s == "10108":
        return True
    if s.isdigit():
        return workflow_info["statuses"].get(s) == "Published"
    return s.lower() == "published"


def compute_published_share_done_terminal_snapshot(df, end_date_ts, workflow_info):
    """
    Published count / (Published + Duplicate + Resolved - No Change) on current Status,
    same population as Status Distribution: Created Date <= end_date_ts, excluding inferred statuses.
    """
    if "Status" not in df.columns or "Created Date" not in df.columns:
        return {
            "published": 0,
            "duplicate": 0,
            "rnc": 0,
            "denom": 0,
            "pct": None,
            "population_note": "Missing Status or Created Date column.",
        }
    issues_mask = df["Created Date"] <= end_date_ts
    sub = df.loc[issues_mask, "Status"]
    sub = sub.dropna()
    sub = sub[~sub.isin(["Unresolved", "Resolved (not applied)"])]
    published = duplicate = rnc = 0
    done_names = set(workflow_info["done_terminal_status_names"])
    for val in sub:
        canon = _normalize_status_value_for_done_share(val, workflow_info)
        if canon in done_names:
            if canon == "Published":
                published += 1
            elif canon == "Duplicate":
                duplicate += 1
            elif canon == "Resolved - No Change":
                rnc += 1
    denom = published + duplicate + rnc
    pct = (100.0 * published / denom) if denom else None
    note = (
        f"Population matches **Status Distribution** for the primary period end "
        f"({end_date_ts.strftime('%B %d, %Y')} UTC): issues with Created Date on or before that "
        f"instant, excluding inferred statuses **Unresolved** and **Resolved (not applied)**."
    )
    return {
        "published": published,
        "duplicate": duplicate,
...
def compute_published_share_done_terminal_cohort(df, period_str, workflow_info):
    """
    Cohort: first-ever Done-terminal transition in [start, end] of period_str.
    Numerator: current Status is Published.
    """
    start_date, end_date, label = parse_time_period(period_str)
    out = {
        "cohort_size": 0,
        ...
        "label": label,
        "published_status_no_first_done": 0,
    }
    if "First Done Terminal Date" not in df.columns:
        out["missing_history"] = True
        return out
    col = df["First Done Terminal Date"]
    if col.isna().all():
        out["missing_history"] = True
        return out
    ...
    pub_no_hist = df["Status"].apply(lambda x: _status_is_published(x, workflow_info)) & col.isna()
    out["published_status_no_first_done"] = int(pub_no_hist.sum())

    mask = col.notna() & (col >= start_date) & (col <= end_date)
    cohort = df.loc[mask]
    out["cohort_size"] = len(cohort)
    out["published_now"] = int(cohort["Status"].apply(lambda x: _status_is_published(x, workflow_info)).sum())
    if out["cohort_size"]:
        out["pct"] = 100.0 * out["published_now"] / out["cohort_size"]
    return out
...
def generate_workflow_diagram():
    ...
    diagram = [
        ...
        "    classDef doing fill:#91caff,stroke:#1677ff;",
        "    ",
        "    class D,G,I terminal;",
        "    class E,H doing;",
        "```"
    ]
...
def generate_report(df, analysis_periods, staff_list, spec_filter=None):
    """Generate full markdown report
    
    Args:
        df: DataFrame with issue data
        analysis_periods: List of period strings to analyze
        staff_list: List of staff display names to exclude from reporter analysis
        spec_filter: Optional specification name to filter by (for title display)
    """
    md = []
    ...
    if spec_filter:
        # Get the actual specification name(s) from the filtered data
        spec_column = 'Specification Display Name'
        if spec_column in df.columns:
            unique_specs = df[spec_column].dropna().unique()
            if len(unique_specs) == 1:
                title += f" - {unique_specs[0]}"
            elif len(unique_specs) > 1:
                title += f" - {len(unique_specs)} Specifications"
        else:
            title += f" - Filtered by '{spec_filter}'"
    
    md.append(f"{title}\n")
    md.append(f"> **Analysis Period:** {human_readable_period}\n")
    if spec_filter:
        md.append(f"> **Specification Filter:** {spec_filter}\n")
    ...
    md.append("Workflow Notes:\n")
    md.append("\n- **White boxes** represent input processing statuses (Deciding states)")
    md.append("- **Blue boxes** represent statuses where work is happening (Doing states) to change the specification related to the issue and will eventually lead to it being 'Published'")
    md.append("- **Green boxes** represent terminal statuses (Done states)")
    ...
    if not times_all.empty:
        ...
        band = get_performance_band_for_display(p80_all, display_decimals=0)
    else:
    ...
    snap_primary = compute_published_share_done_terminal_snapshot(df, end_date, workflow_info)
    spec_change_pct = snap_primary["pct"]
    spec_change_str = f"{spec_change_pct:.1f}%" if spec_change_pct is not None else "N/A"
    md.append(
        "- **✅ Done Issues Resulting in Specification Change:** "
        f"{spec_change_str}"
    )
    md.append("")
    ...
    for period in analysis_periods:
        ...
        if res_metrics['count'] > 0:
            ...
            band = get_performance_band_for_display(p80_val, display_decimals=0) if p80_val is not None else "N/A"
        ...
        cohort_info = compute_published_share_done_terminal_cohort(df, period, workflow_info)
        md.append(f"### Done-terminal cohort in {label}\n")
        md.append("| Period | First Done-terminal transitions | Now Published | % |")
        md.append("|--------|-----------------------------------|---------------|---|")
        if cohort_info.get("missing_history"):
            md.append(f"| {label} | N/A | N/A | N/A |")
        else:
            cs = cohort_info["cohort_size"]
            pn = cohort_info["published_now"]
            pct = cohort_info["pct"]
            pct_s = f"{pct:.1f}%" if pct is not None else "N/A"
            md.append(f"| {label} | {format_count(cs)} | {format_count(pn)} | {pct_s} |")
        md.append("")
        md.append("")
    ...
    for period in analysis_periods:
        ...
        for tri in tri_periods:
            ...
            # Get basic metrics
            _, n, r, b, _, _, _ = get_tri_metrics(df, tri)
            ...
            # Calculate resolution count for this tri period (issues resolved in this period)
            # Use First Resolved Date if available, otherwise use Resolution Date for Tempo
            if 'First Resolved Date' in df.columns and df['First Resolved Date'].notna().any():
                resolved_mask = df['First Resolved Date'].notna()
                resolved_in_tri = resolved_mask & (df['First Resolved Date'] >= tri_start_date) & (df['First Resolved Date'] <= tri_end_date)
                resolution_times = (
                    df.loc[resolved_in_tri, 'First Resolved Date'] - df.loc[resolved_in_tri, 'Created Date']
                ).dt.total_seconds() / 86400.0
            else:
                resolved_mask = df['Resolution Date for Tempo'].notna()
                resolved_in_tri = resolved_mask & (df['Resolution Date for Tempo'] >= tri_start_date) & (df['Resolution Date for Tempo'] <= tri_end_date)
                resolution_times = df.loc[resolved_in_tri, 'days_to_resolution']
            ...
            tri_data.append((tri_label, resolution_metrics))
            tri_cohort_rows.append(
                (tri_label, compute_published_share_done_terminal_cohort(df, tri, workflow_info))
            )
            md.append(f"| {tri_label} | {format_count(n)} | {format_count(b)} | {format_count(doing_count)} | {format_count(r)} | {format_count(done_count)} |")
        
        md.append("")
        
        md.append(f"### Done-terminal cohort by trimester within {label}\n")
        md.append("| Period | First Done-terminal transitions | Now Published | % |")
        md.append("|--------|-----------------------------------|---------------|---|")
        for tri_label, cinfo in tri_cohort_rows:
            if cinfo.get("missing_history"):
                md.append(f"| {tri_label} | N/A | N/A | N/A |")
            else:
                cs = cinfo["cohort_size"]
                pn = cinfo["published_now"]
                pct = cinfo["pct"]
                pct_s = f"{pct:.1f}%" if pct is not None else "N/A"
                md.append(f"| {tri_label} | {format_count(cs)} | {format_count(pn)} | {pct_s} |")
        md.append("")
        ...
        for tri_label, resolution_metrics in tri_data:
            if resolution_metrics['count'] > 0:
                ave_str = format_number(resolution_metrics['ave'], decimals=0)
                med_str = format_number(resolution_metrics['median'], decimals=0)
                p80_str = format_number(resolution_metrics['p80'], decimals=0)
                band = get_performance_band_for_display(resolution_metrics['p80'], display_decimals=0)
            else:
                ave_str = med_str = p80_str = "N/A"
                band = "N/A"
            
            md.append(f"| {tri_label} | {format_count(resolution_metrics['count'])} | {ave_str} | {med_str} | {p80_str} | {band} |")
        
        md.append("")
        
        # Table 3: Application Time Data
        md.append(f"### 🟢 Time to (Resolved) Issue Being Applied in Specification in {label}\n")
        md.append("| Period | Count | Ave (days) | Median (days) |")
        md.append("|--------|-------|------------|---------------|")
        ...
        for tri in tri_periods:
            ...
            application_count = len(application_times)
            ...
            md.append(f"| {tri_label} | {format_count(application_count)} | {app_ave_str} | {app_med_str} |")
    ...
    # Reporter Breakdown by sub-period
    md.append("### Reporter Breakdown\n")
    md.append("| Period | Total Reporters | New Reporters | % New Reporters |")
    md.append("|--------|------------------|----------------|-----------------|")
    
    # For each analysis period, break down by sub-periods
    for period in analysis_periods:
        ...
        for tri in tri_periods:
            ...
            reporter_data = analyze_submitters(df, tri, staff_list)
            
            # Calculate percentage with proper precision
            percent_new = f"{reporter_data['new_reporter_percent']:.1f}%"
            
            md.append(f"| {tri_label} | {format_count(reporter_data['total_reporters_in_period'])} | {format_count(reporter_data['total_new_reporters'])} | {percent_new} |")
    ...
    if has_applier_data:
        ...
        # Applier Breakdown by sub-period
        md.append("### Applier Breakdown\n")
        md.append("| Period | Total Appliers | New Appliers | % New Appliers |")
        md.append("|--------|----------------|--------------|----------------|")
        ...
        for period in analysis_periods:
            ...
            for tri in tri_periods:
                _, _, tri_label = parse_time_period(tri)
                applier_data = analyze_appliers(df, tri, staff_list)
                
                if applier_data is not None:
                    # Calculate percentage with proper precision
                    percent_new = f"{applier_data['new_applier_percent']:.1f}%"
                    
                    md.append(f"| {tri_label} | {format_count(applier_data['total_appliers_in_period'])} | {format_count(applier_data['total_new_appliers'])} | {percent_new} |")
    ...
    def render_breakdown(title, column):
            ...
            for category in categories:
                ...
                for realm in realms:
                    ...
                    for period in analysis_periods:
                        ...
                        # Count resolved issues used for stats
                        resolved_count_for_stats = len(times) if not times.empty else 0
                        ...
                        if not times.empty and len(times) > 0:
                            ...
                            band = get_performance_band_for_display(p80, display_decimals=0)
                        else:
                            ave_str = med_str = p80_str = "N/A"
                            band = "N/A"
                        ...
                        rows_data.append({
                            ...
                            'res_p80': p80_str,
                            'res_perf': band,
                            'resolved_count_for_stats': resolved_count_for_stats,
                            'app_ave': app_ave_str,
                            'app_med': app_med_str,
        ...
        if column == "Specification Display Name":
            md.append(f"| {column} | Realm | Period | Count | Ave (days) | Median (days) | P80 (days) | Performance |")
            md.append("|" + "-" * len(column) + "|-------|--------|-------|------------|---------------|------------|------------|")
            for row in rows_data:
                md.append(f"| {row['category']} | {row['realm']} | {row['period']} | {format_count(row['resolved_count_for_stats'])} | {row['res_ave']} | {row['res_med']} | {row['res_p80']} | {row['res_perf']} |")
        else:
            md.append(f"| {column} | Period | Count | Ave (days) | Median (days) | P80 (days) | Performance |")
            md.append("|" + "-" * len(column) + "|--------|-------|------------|---------------|------------|------------|")
            for row in rows_data:
                md.append(f"| {row['category']} | {row['period']} | {format_count(row['resolved_count_for_stats'])} | {row['res_ave']} | {row['res_med']} | {row['res_p80']} | {row['res_perf']} |")
...
def main():
    ...
    parser.add_argument("--spec",
                    help="Filter analysis to a specific specification (filters by 'Specification Display Name' column)")
    ...
    if args.spec:
        ...
        if spec_column not in df.columns:
            print(f"ERROR: Cannot filter by specification - '{spec_column}' column not found in input file.")
            print("Available columns:", ', '.join(df.columns.tolist()))
            return
        
        initial_count = len(df)
        available_specs = sorted(df[spec_column].dropna().unique())
        
        # Filter to matching specification (case-insensitive partial match)
        # Use regex=False to treat the pattern as a literal string (handles parentheses, etc.)
        df = df[df[spec_column].astype(str).str.contains(args.spec, case=False, na=False, regex=False)]
        filtered_count = len(df)
        
        if filtered_count == 0:
            print(f"ERROR: No issues found matching specification '{args.spec}'.")
            print(f"\nSearching for specifications containing '{args.spec}'...")
            
            # Try to find similar matches to help the user
            spec_lower = args.spec.lower()
            similar_specs = [s for s in available_specs if spec_lower in str(s).lower()]
            
            if similar_specs:
                print(f"Found {len(similar_specs)} similar specification(s):")
                for spec in similar_specs[:10]:
                    print(f"  - {spec}")
                if len(similar_specs) > 10:
                    print(f"  ... and {len(similar_specs) - 10} more")
            else:
                print(f"\nAvailable specifications (showing first 30):")
                for i, spec in enumerate(available_specs[:30], 1):
                    print(f"  {i}. {spec}")
                if len(available_specs) > 30:
                    print(f"  ... and {len(available_specs) - 30} more")
                print(f"\nTotal: {len(available_specs)} unique specifications")
            return
        
        print(f"Filtered to specification '{args.spec}': {filtered_count} issues (from {initial_count} total)")
    ...
    report = generate_report(df, args.periods, staff_list, spec_filter=args.spec)
