...
def process_pages():
    ...
    for space_key in SPACE_KEYS:
        ...
        # Fetch all page IDs for the space
        ...
        for page_id in space_page_ids:
            ...
            if page:
                ...
                images_dir = os.path.join(space_output_dir, "images")
                updated_content, _ = extract_and_export_diagrams_and_attachments(content, page_id, images_dir)
                ...
                output_path = os.path.join(space_output_dir, f"{sanitized_title}.md")
    ...
    # Add pages from parent page IDs and their children
    for parent_id in PARENT_PAGE_IDS:
        print(f"Fetching child pages for parent page ID: {parent_id}")
        child_page_ids = get_child_page_ids(parent_id)
        all_page_ids.extend(child_page_ids)

    # Process all page IDs as before
    for page_id in all_page_ids:
        # Existing logic to fetch, convert, and save as Markdown
        ...
        if page:
            ...
            # Process Draw.io diagrams and update the content inline
            images_dir = os.path.join(misc_output_dir, "images")
            updated_content, _ = extract_and_export_diagrams_and_attachments(content, page_id, images_dir)
            ...
            output_path = os.path.join(misc_output_dir, f"{sanitized_title}.md")
            ...
            with open(output_path, "w") as md_file:
                md_file.write(markdown_content)
            print(f"Converted page {title} to Markdown in miscellaneous directory.")
