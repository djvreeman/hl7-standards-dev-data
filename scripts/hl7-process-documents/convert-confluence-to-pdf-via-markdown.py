# convert-confluence-to-pdf-via-markdown.py
#
# Usage:
#   python convert-confluence-to-pdf-via-markdown.py \
...
#   pages to PDF format using markdownify as an intermediary step. This approach
#   is much more memory-efficient than direct HTML-to-PDF conversion.
#
#   Each space has a config in data/config/spaces/<space_key>.yaml that defines 
#   which pages or parent pages to include or exclude. Includes override excludes.
#
...
#   - Uses markdownify for memory-efficient HTML-to-Markdown conversion
#
...
if not WEASYPRINT_AVAILABLE and not PDFKIT_AVAILABLE:
    ...
    print("    pip install weasyprint pdfkit markdownify")
    print("    brew install cairo pango gdk-pixbuf libffi (macOS)")
    exit(1)
...
def aggressive_memory_cleanup():
    ...
    for module in list(sys.modules.keys()):
        if module.startswith('weasyprint') or module.startswith('pdfkit'):
            try:
...
def create_attachments_section_markdown(attachments, page_dir, page_attachments_dir):
    """Create Markdown section for attachments with relative links"""
    if not attachments:
        return ""
    ...
    markdown_content = "\n\n## 📎 Attachments\n\n"
    markdown_content += "This page contains the following attachments:\n\n"
    
    for file_type, type_attachments in sorted(attachments_by_type.items()):
        markdown_content += f"### {file_type}s\n\n"
        ...
        for att in sorted(type_attachments, key=lambda x: x['filename']):
            ...
            markdown_content += f"- **{att['filename']}**\n"
            markdown_content += f"  - 📏 Size: {att['file_size']}\n"
            markdown_content += f"  - 📅 Downloaded: {att['download_date']}\n"
            markdown_content += f"  - 👤 Author: {att['original_author']}\n"
            markdown_content += f"  - 🔗 [Open File]({rel_path})\n\n"
...
def post_process_markdown(markdown_content):
    """Post-process markdown content to clean up common issues"""
...
def create_pdf_from_markdown(markdown_content, output_path, title):
    """Convert Markdown content to PDF with robust memory management and fallbacks"""
    # Check content size and truncate if too large
    content_size = len(markdown_content.encode('utf-8'))
    max_content_size = 5 * 1024 * 1024  # Reduced to 5MB limit
    
    if content_size > max_content_size:
        log_message(f"⚠️ Markdown content too large ({content_size / 1024 / 1024:.1f}MB), truncating...")
        # Truncate content to prevent memory issues
        markdown_content = markdown_content[:max_content_size // 2] + "\n\n[Content truncated due to size]"
    ...
    try:
        ...
        # Simple fallback if markdown library not available
...
def process_page(page_id, base_url, token, output_dir, shared_attachments_dir, tree=None, page_lookup=None, parent_lookup=None, use_pdfkit_only=False, skip_on_error=False, max_memory_mb=200):
    ...
    log_message(f"🔄 Converting HTML to Markdown...")
    markdown_content = markdownify.markdownify(content_with_links, heading_style="ATX")
    markdown_content = post_process_markdown(markdown_content)
    
    # Add attachments section with relative paths from page directory
    attachments_section = create_attachments_section_markdown(page_attachments, page_dir, page_attachments_dir)
    ...
    try:
        ...
        if current_memory and current_memory > max_memory_mb:
            log_message(f"⚠️ Memory usage too high ({current_memory:.1f}MB > {max_memory_mb}MB), skipping PDF creation")
            if skip_on_error:
                # Save as markdown instead
                markdown_filepath = filepath.replace('.pdf', '.md')
                with open(markdown_filepath, 'w', encoding='utf-8') as f:
                    f.write(markdown_content)
                log_message(f"📄 Saved as Markdown instead: {markdown_filepath}")
                return
            else:
                log_message(f"❌ Memory limit exceeded, use --skip-on-error to continue")
                return
        
        if create_pdf_from_markdown(markdown_content, filepath, title):
            ...
            # Save as markdown as fallback
            markdown_filepath = filepath.replace('.pdf', '.md')
            with open(markdown_filepath, 'w', encoding='utf-8') as f:
                f.write(markdown_content)
            log_message(f"📄 Saved as Markdown fallback: {markdown_filepath}")
    except MemoryError as e:
        log_message(f"❌ Memory error processing page {page_id}: {e}")
        if skip_on_error:
            # Save as markdown instead
            markdown_filepath = filepath.replace('.pdf', '.md')
            with open(markdown_filepath, 'w', encoding='utf-8') as f:
                f.write(markdown_content)
            log_message(f"📄 Saved as Markdown due to memory error: {markdown_filepath}")
        else:
            raise
    except Exception as e:
        ...
        if skip_on_error:
            ...
            with open(markdown_filepath, 'w', encoding='utf-8') as f:
                f.write(markdown_content)
            log_message(f"📄 Saved as Markdown due to error: {markdown_filepath}")
        else:
            raise
    finally:
        # Perform aggressive memory cleanup after processing each page
...
def main():
    global logger
    parser = argparse.ArgumentParser(description="Export Confluence pages to PDF using markdownify for memory efficiency")
    ...
    parser.add_argument('--skip-on-error', action='store_true', help='Skip pages that cause memory errors')
    parser.add_argument('--max-memory-mb', type=int, default=200, help='Maximum memory usage in MB before skipping page')
    ...
    for filename in os.listdir(args.spaces_dir):
        ...
        if not args.dry_run:
            ...
            for pid in sorted(final_ids):
                ...
                if title:
                    try:
                        process_page(pid, base_url, token, space_output_dir, shared_attachments_dir, tree, page_lookup, parent_lookup, args.use_pdfkit_only, args.skip_on_error, args.max_memory_mb)
                    except Exception as e:
